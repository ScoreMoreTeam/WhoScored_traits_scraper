...
class WhoScored(BaseSeleniumReader):
    ...
    def read_schedule(self, force_cache: bool = False) -> pd.DataFrame:
        ...
        for (lkey, skey), stage in df_season_stages.iterrows():
            ...
            if stage_name is not None:
                calendar_filepath = (
                    self.data_dir / f"matches/{lkey}_{skey}_{stage_id}.html"
                )
                logger.info(
    ...
    def parse_player_traits(self, driver):
        """Parse strengths, weaknesses and style of play from a player's WhoScored profile page."""
        traits = {"strengths": [], "weaknesses": [], "style_of_play": []}
        soup = BeautifulSoup(driver.page_source, "html.parser")
        card = soup.find("div", class_="sws-content character-card")
        if not card:
            return traits

        # Strengths
        sec = card.find("div", class_="strengths")
        if sec:
            for item in sec.find_all("div", class_="character"):
                text = item.get_text(strip=True)
                if text:
                    traits["strengths"].append(text)

        # Weaknesses
        sec = card.find("div", class_="weaknesses")
        if sec:
            for item in sec.find_all("div", class_="character"):
                text = item.get_text(strip=True)
                if text:
                    traits["weaknesses"].append(text)

        # Style of Play
        sec = card.find("div", class_="style")
        if sec:
            for item in sec.find_all("div", class_="character"):
                text = item.get_text(strip=True)
                if text:
                    traits["style_of_play"].append(text)

        return traits

    def parse_team_traits(self, driver):
        """Parse strengths, weaknesses and style of play from a team's WhoScored profile page."""
        traits = {"strengths": [], "weaknesses": [], "style_of_play": []}
        soup = BeautifulSoup(driver.page_source, "html.parser")
        card = soup.find("div", class_="sws-content character-card")
        if not card:
    ...
    def parse_match_traits(self, driver):
        """Parse match report traits for both teams from a match page."""
        traits = {
            "home": {"strengths": [], "weaknesses": [], "style": []},
            "away": {"strengths": [], "weaknesses": [], "style": []},
        }
        soup = BeautifulSoup(driver.page_source, "html.parser")
        table = soup.find("table", class_="matchstory")
        if not table:
            return traits
        ...
        for row in rows:
            if "matchstory-typeheader" in row.get("class", []):
                header_text = row.get_text(strip=True).lower()
                if "strength" in header_text:
                    current_section = "strengths"
                elif "weakness" in header_text:
                    current_section = "weaknesses"
                elif "style" in header_text:
                    current_section = "style"
                continue
            elif "matchstory-row" in row.get("class", []):
                cells = row.find_all("td")
                if len(cells) >= 2 and current_section:
                    traits["home"][current_section].append(
                        cells[0].get_text(strip=True)
                    )
                    traits["away"][current_section].append(
